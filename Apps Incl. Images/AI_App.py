...
def ask_AI():
    ...
    #st.write('In particular, the colored areas below have helped me form my guess.') --> only necessary with explanation
    #image_XAI=('Image-XAI-{}.png'.format(str(tempdata["current_roundnumber"]))) --> only necessary with explanation
    #st.image(image_XAI, width=350)
    st.markdown('')
    st.markdown('**Please indicate the extent to which you agree with the following statement.**')
    ...
    with open(filename, "wb") as f: 
        pkle.dump(tempdata, f, pkle.HIGHEST_PROTOCOL)
    if tempdata["trust_in_AI"][-1] != 'not yet selected':
...
if tempdata["current_roundnumber"]==13 and tempdata["status"]>=3 and tempdata["round_status"]==1:
    ...
    fields=['ID-Number', 'Group', 'Gender', 'Age', 'Nationality', 'Highest degree of education', 'Been Berlin', 'Been Hamburg', 'Been Israel', 'Been TelAviv', 'Been Jerusalem', 'Date','1-1-Sel', '1-1-Conf', '1-2-Trust', '1-2-Dec-R', '1-2-Sel', '1-2-Conf', '2-1-Sel', '2-1-Conf', '2-2-Trust', '2-2-Dec-R', '2-2-Sel', '2-2-Conf', '3-1-Sel', '3-1-Conf', '3-2-Trust', '3-2-Dec-R', '3-2-Sel', '3-2-Conf', '4-1-Sel', '4-1-Conf', '4-2-Trust', '4-2-Dec-R', '4-2-Sel', '4-2-Conf','5-1-Sel', '5-1-Conf', '5-2-Trust', '5-2-Dec-R', '5-2-Sel', '5-2-Conf','6-1-Sel', '6-1-Conf', '6-2-Trust', '6-2-Dec-R', '6-2-Sel', '6-2-Conf','7-1-Sel', '7-1-Conf', '7-2-Trust', '7-2-Dec-R', '7-2-Sel', '7-2-Conf','8-1-Sel', '8-1-Conf', '8-2-Trust', '8-2-Dec-R', '8-2-Sel', '8-2-Conf','9-1-Sel', '9-1-Conf', '9-2-Trust', '9-2-Dec-R', '9-2-Sel', '9-2-Conf','10-1-Sel', '10-1-Conf', '10-2-Trust', '10-2-Dec-R', '10-2-Sel', '11-2-Conf','11-1-Sel', '11-1-Conf', '11-2-Trust', '11-2-Dec-R', '11-2-Sel', '11-2-Conf','12-1-Sel', '12-1-Conf', '12-2-Trust', '12-2-Dec-R', '12-2-Sel', '12-2-Conf','13-1-Sel']
    rows=[tempdata["history"]]
    with open('AI_results', 'w') as f:
...
if 13>tempdata["current_roundnumber"]>=1 and tempdata["status"]>=9 and tempdata["round_status"]==2:
    show_selection() 
